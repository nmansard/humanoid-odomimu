...
%The equivalence $\bfq\od\bfv=\bfR\bfv$ 
%and the indistinct usage of the $\Exp()$ and $\Log()$ notations 
This equivalence
...
The deltas $\D\!=\!(\Dp,\Dv,\Dq)$ are defined as the motion (position, velocity, orientation) of a body \wrt this non-rotating free-falling frame. At $t=t_i$, this frame was at position $\bfp_i$ and orientation $\bfq_i$, with velocity $\bfv_i$. At a later time $t=t_j$ the deltas $\D_{ij}=\D(\bfx_i,\bfx_j)$ read
%
...
\end{align}
%
where $\jac{\D}{\bfb_b}$ is the integrated bias Jacobian, which starts at $\jac{\D}{\bfb_b}|_{ii} = {\bf0}_{9\tcross 9}$, and is obtained incrementally using also the chain rule,
%
\begin{align*}
\jac{\D_{ik}}{\bfb_b} 
&= 
