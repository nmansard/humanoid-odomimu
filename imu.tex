...
Additionally, in the following we will refer to the groups $S3$ and $SO(3)$ with the unique name $SO(3)$.
...
Notice that we can define a proper delta $\delta_{jk}$ from the body magnitudes $\bfb_j=(\bfa_j,\bw_j)$ at time $t_j$,
%
...
See \appRef{sec:derivatives_SO3} for details on the development of all non-trivial Jacobian blocks.



\subsubsection{Jacobians of the body magnitudes}

We have from \eqRef{equ:body},% immediately
%
\begin{align}
...
\end{align}
%
We develop the lower-right block $\bfJ^\dth_\bw$ as in \appRef{sec:jac_R3toSO3}.
%
%as follows,
%%
%\begin{align*}
...
Let $\bfQ$ be the covariance of the pre-integrated delta, and $\bfN$ that of the measurement noise. For convenience, we first comute the covariance of the current delta,
%
\begin{align}
...
%%
%\begin{align}
%\jac{\delta}{\bfb_n} = \jac{\delta}{\bfb} \cdot \jac{\bfb}{\bfb_n} = \jac{\delta}{\bfb}
%\end{align}
%
Then,
%
\begin{align}
\bfQ^+ = \jac{\D^+}{\D}\bfQ\,\,\jac{\D^+}{\D}\tr + \jac{\D^+}{\delta}\bfQ_\delta\,\jac{\D^+}{\delta}\tr
~.
\end{align}
