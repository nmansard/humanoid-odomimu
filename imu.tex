...
Delta pre-integration theory was developed \cite{LUPTON-09,FORSTER-16-techrep} to avoid the need of re-integrating all IMU data at each iteration. 
On one hand, new motion magnitudes called \emph{deltas} are defined, which are independent of the initial conditions of orientation and velocity, and thus depend \emph{only} on the IMU data and bias. 
...
and 
3) a simpler approach to all the algebra needed for a rigorous treatment in both the S3 (quaternion) and SO3 (rotation matrix) manifolds, which takes profit of the pipeline segmentation and the chain rule to compute the otherwise combersome Jacobians. 
...
Consider a non-rotating reference frame that is free-falling at the acceleration of gravity $\bfg$, and name it $\cF_g$. 
An ideal (unbiased and noiseless) IMU glued to this frame would measure null accelerations and rotations. 
Any non-null measurements would imply motion of the IMU \wrt  $\cF_g$.

%The deltas $\D\!=\!(\Dp,\Dv,\Dq)$ are defined as the motion (position, velocity, orientation) of a body \wrt this non-rotating free-falling frame. 
At $t=t_i$, we associate $\cF_g$ at $\bfx_i=(\bfq_i,\bfv_i,\bfq_i)$.
At a later time $t=t_j$,  $\cF_g$ has fallen according to $\bfg$, and the state of our moving body is at $\bfx_j=(\bfq_j,\bfv_j,\bfq_j)$.
The motion delta, denoted $\D_{ij}%=\D(\bfx_i,\bfx_j)
$, is defined as the motion (position, velocity, orientation) of a body \wrt  $\cF_g$, that is,
%
...
Interestingly, the deltas form a group under the composition operator, $\D_{ik}\te\D_{ij}\oplus\D_{jk}$, defined by,
%
...
\end{align}
%
described in \eqRef{equ:g_composition}. In the following, we will identify $\D$ with the pre-integrated delta, and $\delta$ with the current delta.
...
We differentiate the delta composition \eqRef{equ:composition_compact} described in \eqRef{equ:g_composition},
%
\begin{subequations}
\begin{align}
...
\end{align}
%
where $\jac{\D}{\bfb_b}$ is the integrated bias Jacobian, computed incrementally using also the chain rule,
%
\begin{align*}
\jac{\D_{ik}}{\bfb_b} 
&= 
