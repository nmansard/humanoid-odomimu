...
Since the quaternion implementation is one of the contributions, in the following we will refer to the groups $S3$ and $SO(3)$ with the unique name $S3$, although everything applies equally to $SO(3)$. 
...
The `plus' operator $\oplus:S3\times\bbR^3\to S3$ (\figRef{fig:manifold}) composes a reference element $\sR\in S3$ with a (often small) rotation specified by a vector of $\bth\in\bbR^3$ that is tangent to the $S3$ manifold at $\sR$, yielding an element $\sS\in S3$, 
%
%\begin{align*}
