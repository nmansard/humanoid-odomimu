...
In case of manifolds, (\ref{eq:error}) becomes $\bfr(\bfx) = (h(\bfx) \oplus \bfv) \ominus \bfz$, with $\bfJ_k = \partial (h_k(\bfx)\ominus\bfz_k)/\partial \Delta \bfx$. The $\oplus$ and $\ominus$ are the addition and subtraction operators on the manifold, as described for each state block intervening in the factor (see \cite{Smith_arv90} and \secRef{sec:imu}).
%
