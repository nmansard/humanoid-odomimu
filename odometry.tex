...
Each factor $k$ requires its own information matrix~$\bfOmega_k$, and its residual function $\bfr_k(\bfx)$. These residuals are detailed hereafter.
...
For the quaternion we implement a residual using the operator $\ominus$ in the $S3$ manifold (see \appRef{sec:derivatives_SO3}),
%
\begin{align}
...
These factors are by far the most complex and the whole of the next section is devoted to describe them in detail.
